...
jq( document ).ready( function() {
    ...
    //jq( '.last select, .standard-form select, #whats-new-form select' ).selecter();
    //cb_enable_select_style();

    // On Single group page, we need to cleanup logged in user nav
    // This is a wrong behaviour on BuddyPress's part
    var $nav_item = jq( '.menu-user-account li:first-child');
    if ( $nav_item.get(0) && $nav_item.attr('id') == 'li-nav-groups' ) {
        $nav_item.remove();
    }
    //On Directory search, add focus class
    jq( '.dir-search input[type="text"]' ).on('focus', function () {
        var $form = jq(this).parents('form');
        $form.addClass('form-focused');
    });

    jq( '.dir-search input[type="text"]' ).on('blur', function () {
        var $form = jq(this).parents('form');
    ...
    // we need to inject css ourself
    if (  typeof  bp !== 'undefined' && undefined != bp.CoverImage ) {
        bp.CoverImage.Attachment.on( 'change:url', function( data ) {
            var url =  data.get('url');//get url from the model
            if ( ! url ) {
                jq( '#item-header' ).css( {
                    'background-image': 'none'
                } );
                return ;//should we remove?
            }
   ...
   // Make BuddyPress notices Closeable
    jq(document).on( 'click', '.cb-close-notice', function() {

        jq('#site-feedback-message').fadeOut( 500 );
        var $message = jq(this).parent().parent();

        $message.fadeOut( 500 );
    } );

	// for Header login button click, if right panel is available, open it
	var panel_login_form = jq( '#panel-login-form' );
	jq ( document ).on( 'click', '.bp-ajaxl', function() {
		if ( panel_login_form.get(0) ) {
			jQuery( '#panel-right-toggle' ).trigger('click');
			return false;
		}
	...
	jq('#aw-whats-new-submit').on( 'click', function() {
		...
		firstrow = jq( 'ul.activity-list li' ).first();
    ...
    jq(document).on( 'change', '#activity-filter-select select', function() {
	...
	jq('div.activity-type-tabs').on( 'click', function(event) {
		...
		//do not filter last tab
       // console.log(target );
		if ( target.hasClass('filter') || target.hasClass('last') || target.hasClass('opt') ) {
            return ;//let the selectbox handler take care of it
	...
	jq('div.activity').on( 'click', function(event) {
		...
		if ( target.parent().hasClass('load-newest') ) {
			...
			jq( '#content ul.activity-list' ).prepend( newest_activities );
	...
	jq('div.item-list-tabs').on( 'click', function(event) {
		...
		if ( 'LI' === target[0].nodeName && !target.hasClass( 'last' ) && ! target.hasClass('opt') ) {
	...
	jq(document).on('change', 'li.filter select', function() {
        jq(this).parents('.sa-stacker').removeClass('sa-open');
		var el,
	...
	jq(document).on( 'click', '#invite-list input', function() {

		var invites_new_template = true,
		...
		if ( invites_new_template ) {
			//disable all input
			$item.parents( 'ul' ).find( 'input' ).prop( 'disabled', true );
		}
	...
	jq(document).on('click', '.group-invites-list a.remove', function() {
		// invites-loop template contains a div with the .invite class
		// We use the existence of this div to check for old- vs new-
		// style templates.
		var invites_new_template = true;//jq('#send-invite-form > .invite').length,
	...
	jq( '.visibility-toggle-link, .current-visibility-level' ).on( 'click', function( event ) {
		event.preventDefault();

        if ( jq(this).parent().hasClass('field-visibility-settings-notoggle') ) {
            return ;
        }
	...
	jq( '.field-visibility-settings-close' ).on( 'click', function( event ) {
		...
		var settings_div = jq( this ).parents('.field-visibility-settings'),
	...
	jq( '.field-visibility-settings .radio' ).on( 'click', 'input', function () {
		var $settings = jq( this ).parents( '.field-visibility-settings' );
		var vis_setting_text = $settings.find( 'input:checked' ).parent().text();

		$settings.hide().removeClass( 'field-visibility-settings-open' )
			.siblings( '.field-visibility-settings-toggle' )
			.children( '.current-visibility-level' ).text( vis_setting_text ).end()
			.show().removeClass( 'field-visibility-settings-hide' );
	});
	...
	jq('#friend-list a.accept, #friend-list a.reject').on( 'click', function() {
		...
		if ( jq(this).hasClass('accept') ) {
			...
			hidden_div = action_div.find('.generic-friendship-reject');
		} else {
			action = 'reject_friendship';
			hidden_div = action_div.find('.generic-friendship-accept');
		}
		...
		function(response) {
			...
			if ( response[0] + response[1] === '-1' ) {
                ...
                hidden_div.show();//on error, we need to display the other action too
			} else {
				button.fadeOut( 100, function() {
                    hidden_div.hide();
                    jq(this).parent('.generic-button').html( response );
				});
			}
		});
	...
	jq('#content').on('click', '.group-button .leave-group', function() {
	...
	jq('#content').on( 'click', '.pending', function() {
		return false;
	});
	...
	jq('#messages-select').on('change', function(){
		var val = jq(this).val();
		jq('#messages-bulk-manage').attr('disabled', val.length <= 0);
		if( val === 'all' ) {
			jq('#select-all-messages').trigger('click');
		}
	});
	...
	jq( document ).on( 'heartbeat-send.buddypress', function( e, data ) {
		...
		if ( jq( '#content ul.activity-list li' ).first().prop( 'id' ) ) {
			// getting the timestamp
			timestamp = jq( '#content ul.activity-list li' ).first().prop( 'class' ).match( /date-recorded-([0-9]+)/ );
	...
	jq( document ).on( 'heartbeat-tick', function( e, data ) {
		...
		if ( jq( '#content ul.activity-list li' ).first().hasClass( 'load-newest' ) ) {
			return;
		}

		jq( '#content ul.activity-list' ).prepend( '<li class="load-newest"><a href="#newest">' + BP_DTheme.newest + '</a></li>' );
	});
	

    //Check the window scroll event.
	if ( SocialPortal.bp_activity_enable_autoload ) {
    ...
    function load_more_activity() {
			...
			var $load_more = jq( $parent ).find( 'li.load-more');
			...
			var load_more_search = bp_get_querystring('s');
			...
			function(response)
			{
				...
				jq('ul.activity-list', $parent ).append(response.contents);

				$load_more.hide();//Hide any "load more" button.
                cb_enable_select_style();
...
function bp_filter_request( object, filter, scope, target, search_terms, page, extras, caller, template ) {
	...
	function(response)
	{
		...
		if ( caller === 'pag-bottom' && jq('.item-list-tabs').length ) {
			var top = jq('.item-list-tabs').parent();
			jq('html,body').animate({scrollTop: top.offset().top}, 'slow', function() {
				jq(target).fadeOut( 100, function() {

					jq(this).html(response);
					 // do stuff after images are loaded here
                    cb_update_items_layout( target );
					jq(this).fadeIn(100);
				});
			});
			...
			jq(target).fadeOut( 100, function() {
				...
				if( object =='invite' ) {
					jq(target).html(response).fadeIn(100);
					jq('.item-list-tabs li.selected').removeClass('loading');
					return ;
				}
 ...
 * @param container
 */
cb_equalheight = function(container) {
    ...
    jQuery(container).imagesLoaded( function() {
        ...
        jQuery('.item-list-equalheight>li', jQuery( container ) ).each(function() {
            var $el = jQuery(this);
            $el.height('auto')
 ...
 * @returns {undefined}
 */
function cb_update_items_layout( target ) {
    var type = SocialPortal.bp_item_list_display_type;
    if ( 'equalheight' == type ) {
        cb_equalheight( target );
    } else if( 'masonry' === type ) {
        cb_update_item_grid( target );
    }
}

//when images are loaded, updated height of columns
jQuery( window ).on(  'load',  function() {
    cb_update_items_layout( 'body' );
} );

//on window resize, we need to update the height again
jQuery( window ).resize( function() {
    //resizing is only required for equal heights
    if ( SocialPortal.bp_item_list_display_type == 'equalheight' ) {
        cb_update_items_layout( 'body' );
    }

} );

function cb_update_item_grid( $target ) {
    var $container = jQuery($target).find('.item-list-masonry');

    $container.imagesLoaded( function () {
        $container.masonry({
            columnWidth: '.item-gradient',
            itemSelector: '.item-gradient'
        });
    });
}
...
/**
 * Enable/re enable the select box styling
 *
 */
function cb_enable_select_style() {
    // is the styling enabled?
    if ( ! SocialPortal.enable_selectbox_styling ) {
        return ;
    }

    //jQuery( 'select' ).not('.sa-field-content select').selecter();
    if( SocialPortal.selectbox_excluded_selector && SocialPortal.selectbox_excluded_selector.length> 0 ){
        jQuery( 'select' ).not(SocialPortal.selectbox_excluded_selector).SumoSelect();
    }
}