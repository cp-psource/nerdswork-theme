		...
		<?php else : ?>
				...
				<div class="folder-action-links">
					<?php if ( current_user_can( 'bp_docs_manage_folders' ) ) : ?>
						<div class="manage-folders-link">
							<a href="<?php bp_docs_manage_folders_url() ?>"><?php _e( 'Ordner verwalten', 'social-portal' ) ?></a>
						</div>
					<?php endif ?>

					<div class="toggle-folders-link hide-if-no-js">
						<a href="#" class="toggle-folders" id="toggle-folders-hide"><?php _e( 'Ordner ausblenden', 'social-portal' ) ?></a>
						<a href="#" class="toggle-folders" id="toggle-folders-show"><?php _e( 'Ordner anzeigen', 'social-portal' ) ?></a>
					</div>
				</div>
			<?php endif; ?>
			...
			<table class="doctable">
				...
				<tr valign="bottom">
					...
					<th scope="column" class="title-cell<?php bp_docs_is_current_orderby_class( 'title' ) ?>">
						<a href="<?php bp_docs_order_by_link( 'title' ) ?>"><?php _e( 'Titel', 'social-portal' ); ?></a>
					</th>

					<?php if ( ! bp_docs_is_started_by() ) : ?>
						<th scope="column" class="author-cell<?php bp_docs_is_current_orderby_class( 'author' ) ?>">
							<a href="<?php bp_docs_order_by_link( 'author' ) ?>"><?php _e( 'Autor', 'social-portal' ); ?></a>
						</th>
					<?php endif; ?>

					<th scope="column" class="created-date-cell<?php bp_docs_is_current_orderby_class( 'created' ) ?>">
						<a href="<?php bp_docs_order_by_link( 'created' ) ?>"><?php _e( 'Erstellt', 'social-portal' ); ?></a>
					</th>

					<th scope="column" class="edited-date-cell<?php bp_docs_is_current_orderby_class( 'modified' ) ?>">
						<a href="<?php bp_docs_order_by_link( 'modified' ) ?>"><?php _e( 'Bearbeitet', 'social-portal' ); ?></a>
					</th>
					...
					<?php if ( ! empty( $_GET['folder'] ) ) : ?>
						<tr class="folder-row">
							...
							<td class="folder-row-name" colspan=10>
								<a href="<?php echo esc_url( bp_docs_get_parent_folder_url() ) ?>" class="up-one-folder"><?php bp_docs_genericon( 'category', 0 ); ?><span class="screen-reader-text"><?php _e( 'Gehe einen Ordner hoch', 'social-portal' ) ?></span><?php _ex( '..', 'einen Ordner hoch', 'social-portal' ) ?></a>
							</td>
						</tr>
					<?php endif ?>
			...
			<?php if ( $has_docs ) : ?>
				<div id="bp-docs-pagination">
					<div id="bp-docs-pagination-count">
						<?php printf( __( 'Zeige %1$s-%2$s von %3$s Docs', 'social-portal' ), bp_docs_get_current_docs_start(), bp_docs_get_current_docs_end(), bp_docs_get_total_docs_num() ); ?>
					</div>
			...
			<?php else : ?>
				<?php if ( bp_docs_current_user_can_create_in_context() ) : ?>
					<p class="no-docs"><?php printf( __( 'Für diese Ansicht sind keine Dokumente vorhanden. Warum nicht <a href="%s">eines erstellen</a>?', 'social-portal' ), bp_docs_get_create_link() ); ?>
				<?php else : ?>
					<p class="no-docs"><?php _e( 'Für diese Ansicht sind keine Dokumente vorhanden.', 'social-portal' ); ?></p>
				<?php endif; ?>
			<?php endif; ?>
		<?php endif; ?>
