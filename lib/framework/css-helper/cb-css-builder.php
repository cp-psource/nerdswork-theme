...
/**
 * @credit Make theme
 * Modified for WMS N@W Theme
 */

/**
 * Singleton to collect and print CSS based on user selections.
 *
...
class CB_CSS_Builder {
	...
	public function add( $data ) {
		 ...
		 * @param array    $data    The selectors and properties to add to the CSS.
		 */
		$data  = apply_filters( 'cb_css_builder_add_rule', $data );
	...
	private function parse_declarations( $declarations, $tab = '' ) {
		 ...
		 * routines are implemented when the developer calls `cb_get_css_builder()->add()`. Because every property value has
		...
		foreach ( $declarations as $property => $value ) {
			 ...
			 */
			$output .= apply_filters( 'cb_parse_css_declaration', $parsed_value, $property, $value, $t, $n );
		}
		 ...
		 */
		return apply_filters( 'cb_parse_css_declarations', $output, $declarations, $tab );
	}
}
...
function cb_get_css_builder() {
	return CB_CSS_Builder::get_instance();
}
